...
Направление выбирать не нужно: язык определяет сама переводящая модель и сообщает его
первой строкой ответа (`LANG: sr` или `LANG: ru`). От этого зависят и акцент диктора,
и флаг в подписи. Распознаванию язык намеренно не подсказывается — жёсткая подсказка
заставляет Whisper слышать сербскую речь как исковерканную русскую.
...
| 2 | Распознаёт речь, язык определяется автоматически (`whisper-1`) | `bot/speech.py` |
| 3 | Определяет язык и переводит в обратную сторону (OpenAI **или** Anthropic) | `bot/translation.py` |
| 4 | Озвучивает перевод на нужном языке (`gpt-4o-mini-tts`, OGG/Opus) | `bot/speech.py` |
...
разбор конфига, разбор метки направления, очистка ответа модели от лишних кавычек
и подписей, а также запасной текстовый ответ при запрете голосовых.
